...
function buildFerryLink(from: string, to: string, date: string): string {
  ...
  // ????? ??????? ??????? ??? ??????? ???? ?????? Balearia
  const baleariaNames: Record<string, string> = {
    '????': 'Tangier',
    '???? ???????': 'Tanger Med',
    '???? ???????': 'Tanger Ville',
    '???????': 'Nador',
  ...
  // ????? ??????? ?????????? ???????
  const fixEnglishNames: Record<string, string> = {
  ...
  let fromName = baleariaNames[fLower] || from;
  let toName = baleariaNames[tLower] || to;
  
  const fromLower = fromName.toLowerCase();
  const toLower = toName.toLowerCase();
  fromName = fixEnglishNames[fromLower] || fromName;
  toName = fixEnglishNames[toLower] || toName;
  
  const encodedFrom = encodeURIComponent(fromName);
  const encodedTo = encodeURIComponent(toName);
  
  return `https://www.balearia.com/en/search-v2?origin=${encodedFrom}&destination=${encodedTo}&departure_date=${d}&passengers=1`;
}
