...
function buildFerryLink(from: string, to: string, date: string): string {
  ...
  let fromEnglish = translationMap[from] || from;
  let toEnglish = translationMap[to] || to;
  
  const encodedFrom = encodeURIComponent(fromEnglish);
  const encodedTo = encodeURIComponent(toEnglish);
