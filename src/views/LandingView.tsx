...
export function LandingView({ onOpenApp, lang, setLang, onOpenPrivacy, adminTrigger }: LandingViewProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Announcement Bar */}
      <a href="https://aviasales.tpx.gr/s8LtyZGl" target="_blank" rel="noopener" onClick={() => trackClick('https://aviasales.tpx.gr/s8LtyZGl')}
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden border-b border-border">
        <img src={IMAGES.hero} alt="Travel" className="absolute inset-0 w-full h-full object-cover object-center" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/40 to-black/20" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 py-32 text-left">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="inline-block px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white font-bold text-xs uppercase tracking-widest mb-6">
            {t.hero_badge}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <button onClick={() => onOpenApp()}
              className="px-8 py-4 rounded-full bg-gradient-to-r from-[#FF6B35] to-[#FFD23F] text-white font-black text-lg hover:shadow-[0_10px_30px_rgba(255,107,53,0.5)] transition-all flex items-center gap-2 transform hover:-translate-y-1">
              🚀 {t.hero_btn_start}
            </button>
        ...
        <div className="flex gap-8 whitespace-nowrap px-4 animate-marquee">
          {[1, 2].map(k => (
            <div key={k} className="flex gap-12 items-center">
              <button onClick={() => onOpenApp('flight')} className="flex items-center gap-2 text-foreground hover:text-primary transition-colors font-bold uppercase tracking-wider text-sm"><Plane size={16} className="text-primary" /> {t.cheap_flights}</button>
              <button onClick={() => onOpenApp('hotel')} className="flex items-center gap-2 text-foreground hover:text-secondary transition-colors font-bold uppercase tracking-wider text-sm"><Building2 size={16} className="text-secondary" /> {t.top_hotels}</button>
              <button onClick={() => onOpenApp('experience')} className="flex items-center gap-2 text-foreground hover:text-accent transition-colors font-bold uppercase tracking-wider text-sm"><Ticket size={16} className="text-accent" /> {t.experiences}</button>
              <a href="https://searadar.tpx.gr/PRzRZqAz" target="_blank" rel="noopener" onClick={() => trackClick('https://searadar.tpx.gr/PRzRZqAz')} className="flex items-center gap-2 text-foreground hover:text-info transition-colors font-bold uppercase tracking-wider text-sm"><Ship size={16} className="text-info" /> {t.cruises}</a>
              <button onClick={() => onOpenApp('taxi')} className="flex items-center gap-2 text-foreground hover:text-primary transition-colors font-bold uppercase tracking-wider text-sm"><Car size={16} className="text-primary" /> {t.car_rental}</button>
              <button onClick={() => onOpenApp('bus')} className="flex items-center gap-2 text-foreground hover:text-secondary transition-colors font-bold uppercase tracking-wider text-sm"><Train size={16} className="text-secondary" /> {t.trains_buses}</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <section id="services" className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { svc: 'flight', img: IMAGES.flight, icon: <Plane size={20}/>, label: t.cheap_flights, color: '#FF6B35' },
            { svc: 'hotel', img: IMAGES.hotel, icon: <Building2 size={20}/>, label: t.top_hotels, color: '#00C9B1' },
            { svc: 'experience', img: IMAGES.tour, icon: <Ticket size={20}/>, label: t.experiences, color: '#FFD23F' },
            { svc: 'taxi', img: IMAGES.transfer, icon: <Car size={20}/>, label: t.car_rental, color: '#4ECDC4' },
          ].map(({ svc, img, icon, label, color }) => (
            <button key={svc} onClick={() => onOpenApp(svc)}
              className="group relative h-48 md:h-64 rounded-3xl overflow-hidden shadow-lg border border-border text-left">
              <img src={img} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={label} />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
              <div className="absolute bottom-6 left-6">
                <div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur text-white flex items-center justify-center mb-3 shadow-lg">{icon}</div>
                <h3 className="text-xl font-black text-white drop-shadow-md">{label}</h3>
              </div>
            </button>
          ))}

          <a href="https://searadar.tpx.gr/PRzRZqAz" target="_blank" rel="noopener" onClick={() => trackClick('https://searadar.tpx.gr/PRzRZqAz')}
            className="group relative h-48 md:h-64 rounded-3xl overflow-hidden shadow-lg border border-border text-left md:col-span-2">
            <img src={IMAGES.cruise} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90" alt="Cruises" />
          ...
          <a href="https://hotels.tpx.gr/84T1xJ3l" target="_blank" rel="noopener" onClick={() => trackClick('https://hotels.tpx.gr/84T1xJ3l')}
            className="group relative h-48 md:h-64 rounded-3xl overflow-hidden shadow-lg border border-border text-left md:col-span-2">
            <img src={IMAGES.luxury} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90" alt="Luxury" />
            <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors"></div>
            <div className="absolute bottom-6 left-6 z-10">
      ...
      <section id="how" className="py-24 bg-surface border-y border-border shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-12 relative">
            ...
            {[
              { num: '01', title: t.step1_title, desc: t.step1_desc, border: 'border-primary', color: 'text-primary', shadow: 'shadow-[0_10px_30px_rgba(255,107,53,0.2)]' },
              { num: '02', title: t.step2_title, desc: t.step2_desc, border: 'border-accent', color: 'text-accent', shadow: 'shadow-[0_10px_30px_rgba(255,210,63,0.2)]' },
              { num: '03', title: t.step3_title, desc: t.step3_desc, border: 'border-secondary', color: 'text-secondary', shadow: 'shadow-[0_10px_30px_rgba(0,201,177,0.2)]' },
            ].map(({ num, title, desc, border, color, shadow }) => (
              <div key={num} className="relative z-10 flex flex-col items-center text-center">
                <div className={`w-24 h-24 rounded-full bg-surface border-[6px] ${border} flex items-center justify-center text-3xl font-black ${color} mb-6 ${shadow}`}>{num}</div>
                <h3 className="text-xl font-bold text-foreground mb-3">{title}</h3>
                <p className="text-muted font-medium">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="destinations" className="py-24 px-4 max-w-7xl mx-auto">
        <a href="https://getyourguide.tpx.gr/Gfoy3dVm" target="_blank" rel="noopener" onClick={() => trackClick('https://getyourguide.tpx.gr/Gfoy3dVm')}
          className="block bg-gradient-to-r from-[#FFD23F] to-[#FF6B35] rounded-3xl p-8 md:p-12 mb-16 text-center transform hover:scale-[1.02] transition-transform shadow-xl">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <a href="https://aviasales.tpx.gr/s8LtyZGl" target="_blank" rel="noopener" onClick={() => trackClick('https://aviasales.tpx.gr/s8LtyZGl')}
            className="md:col-span-2 group rounded-3xl overflow-hidden relative h-[400px] shadow-lg">
          ...
          <div className="space-y-6">
            <a href="https://booking.tpx.gr/XkdJ1mCf" target="_blank" rel="noopener" onClick={() => trackClick('https://booking.tpx.gr/XkdJ1mCf')}
              className="block group rounded-3xl overflow-hidden relative h-[188px] shadow-lg">
            ...
            <a href="https://discovercars.tpx.gr/y9NI2Vf2" target="_blank" rel="noopener" onClick={() => trackClick('https://discovercars.tpx.gr/y9NI2Vf2')}
              className="block group rounded-3xl overflow-hidden relative h-[188px] shadow-lg">
      ...
      <div className="border-y border-border py-10 bg-gradient-to-b from-surface to-[#FFF8F0]">
        ...
        <div className="flex gap-12 whitespace-nowrap px-4 animate-marquee opacity-70">
          {[1, 2, 3].map(k => (
            <div key={k} className="flex gap-12 items-center">
              {PARTNERS.map(p => (
                <span key={p.name} className="font-serif font-bold text-xl text-foreground">{p.name}</span>
              ))}
            </div>
          ))}
        </div>
      ...
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4">
          <h2 className="text-4xl md:text-6xl font-serif font-black text-white mb-6 drop-shadow-lg">
            Ready to travel <br className="md:hidden"/><em className="text-white underline decoration-wavy decoration-[#00C9B1]">smarter?</em>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div>
              ...
              <ul className="space-y-3">
                ...
                <li><button onClick={() => onOpenApp('bus')} className="text-white/60 hover:text-white transition-colors text-sm font-medium">{t.trains_buses}</button></li>
              </ul>
            </div>
            <div>
              <h4 className="text-white font-bold mb-4 uppercase tracking-wider text-sm">Top Partners</h4>
              <ul className="space-y-3">
                <li><a href="https://booking.tpx.gr/XkdJ1mCf" target="_blank" rel="noopener" className="text-white/60 hover:text-white transition-colors text-sm font-medium">Booking.com</a></li>
                <li><a href="https://trip.tpx.gr/WcnMHB2l" target="_blank" rel="noopener" className="text-white/60 hover:text-white transition-colors text-sm font-medium">Trip.com</a></li>
                <li><a href="https://getyourguide.tpx.gr/Gfoy3dVm" target="_blank" rel="noopener" className="text-white/60 hover:text-white transition-colors text-sm font-medium">GetYourGuide</a></li>
                <li><a href="https://aviasales.tpx.gr/s8LtyZGl" target="_blank" rel="noopener" className="text-white/60 hover:text-white transition-colors text-sm font-medium">Aviasales</a></li>
                <li><a href="https://klook.tpx.gr/yBUxcarT" target="_blank" rel="noopener" className="text-white/60 hover:text-white transition-colors text-sm font-medium">Klook</a></li>
              </ul>
            </div>
            <div>
