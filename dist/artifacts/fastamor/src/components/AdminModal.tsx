...
import { getStats, getActiveRef } from '@/lib/tracking';
...
export function AdminModal({ isOpen, onClose }: AdminModalProps) {
  ...
  const handleCopy = () => {
    if (!campName.trim()) return alert("Enter source name");
              ...
              {tab === 'marketing' && (
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="bg-background border border-border rounded-2xl p-6 space-y-4 shadow-sm">
                    <div>
                      <label className="block text-sm font-bold text-foreground mb-2">Campaign/Source Name</label>
                      <input 
                        type="text" 
                        value={campName}
                    ...
                    >
                      <Copy size={18} /> Generate & Copy Link
                    </button>
                  </div>
                </div>
              )}
